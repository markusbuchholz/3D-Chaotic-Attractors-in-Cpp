...
// g++ attractor_newton_leipnik.cpp -o t -I/usr/include/python3.8 -lpython3.8
...
{
    plt::title("Newton-Leipnik attractor ");
...
int main()
{
    auto newton = methodRungeKutta1Diff();
    auto xX = std::get<0>(newton);
    auto yY = std::get<1>(newton);
    auto zZ = std::get<2>(newton);
    plot3D(xX, yY, zZ);
    plot2D(xX, yY);
}