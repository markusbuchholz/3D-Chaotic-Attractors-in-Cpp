...
// g++ attractor_chen_lee.cpp -o t -I/usr/include/python3.8 -lpython3.8
...
{
    plt::title("Nose-Hoover attractor ");
...
int main()
{
    auto nose = methodRungeKutta1Diff();
    auto xX = std::get<0>(nose);
    auto yY = std::get<1>(nose);
    auto zZ = std::get<2>(nose);
    plot3D(xX, yY, zZ);
    plot2D(xX, yY);
}