...
// g++ attractor_finance.cpp -o t -I/usr/include/python3.8 -lpython3.8
...
int main()
{
    auto finance = methodRungeKutta1Diff();
    auto xX = std::get<0>(finance);
    auto yY = std::get<1>(finance);
    auto zZ = std::get<2>(finance);
    plot3D(xX, yY, zZ);
    plot2D(xX, yY);
}