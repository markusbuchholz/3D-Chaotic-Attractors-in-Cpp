...
// g++ attractor_bouali.cpp -o t -I/usr/include/python3.8 -lpython3.8
...
int main()
{
    auto bouali = methodRungeKutta1Diff();
    auto xX = std::get<0>(bouali);
    auto yY = std::get<1>(bouali);
    auto zZ = std::get<2>(bouali);
    plot3D(xX, yY, zZ);
    plot2D(xX, yY);
}