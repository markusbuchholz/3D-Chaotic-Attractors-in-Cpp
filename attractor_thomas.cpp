...
// g++ attractor_thomas.cpp -o t -I/usr/include/python3.8 -lpython3.8
...
int main()
{
    auto thomas = methodRungeKutta1Diff();
    auto xX = std::get<0>(thomas);
    auto yY = std::get<1>(thomas);
    auto zZ = std::get<2>(thomas);
    plot3D(xX, yY, zZ);
    plot2D(xX, yY);
}